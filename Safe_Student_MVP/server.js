...
function guardianInvitationView(invitation) {
  return { id: invitation.id, name: invitation.name, email: invitation.email,
    phone: invitation.phone, relationship: invitation.relationship,
...
function issueGuardianInvitation(db, guardian, studentId) {
  ...
  Object.assign(invitation, { name: guardian.name, phone: guardian.phone,
    relationship: guardian.relationship, codeHash: digestCode(code),
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/guardian/register') {
      ...
      const account = { id: crypto.randomUUID(), name: invitation.name, email, phone: invitation.phone,
        passwordHash: hashPassword(password), role: 'RESPONSAVEL', studentIds, status: 'ATIVO' };
    ...
    if (req.method === 'POST' && url.pathname === '/api/students') {
      ...
      const matchedGuardian = existingGuardian || (guardian && db.users.find(u => normalizeEmail(u.email) === guardian.email && u.role === 'RESPONSAVEL' && u.status === 'ATIVO'));
      if (matchedGuardian && guardian && !existingGuardian && guardian.name.toLocaleLowerCase('pt-BR') !== matchedGuardian.name.toLocaleLowerCase('pt-BR')) {
        return json(res, 409, { error: 'O e-mail já possui responsável com outro nome. Confira o cadastro antes de vincular.' });
      }
    ...
    if (req.method === 'POST' && url.pathname === '/api/guardians/invitations') {
      ...
      const anyAccount = db.users.find(u => normalizeEmail(u.email) === guardian.email);
      if (anyAccount) return json(res, 409, { error: 'O e-mail já possui conta. Utilize "Vincular responsável existente" e confira a identidade.' });
