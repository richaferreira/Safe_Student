...
function renderMetrics(){
  ...
  const cards=[
    ...
    ['i-scan',m.entradasHoje,'Entradas hoje','Registros efetuados hoje',guardian?'students':'presence'],
    ['i-arrow',m.saidasHoje,'Saídas hoje','Registros efetuados hoje','reports'],
...
function renderDashboard(){
  ...
  $('recentAttendance').innerHTML=dashboard.attendance.slice(0,5).map(r=>
    `<div class="timeline-row"><span class="event-icon ${r.type==='SAIDA'?'exit':''}" aria-hidden="true">${r.type==='ENTRADA'?'↘':'↗'}</span><div><strong>${escapeHtml(r.student)}</strong><small>${r.type==='ENTRADA'?'Entrada':'Saída'} · Registrado por token</small></div><span class="event-time">${fmt(r.timestamp)}</span></div>`
...
async function loadGuardians(){
  ...
  $('guardiansTable').innerHTML=d.guardians.length?`<table><thead><tr><th>Responsável</th><th>Contato</th><th>Filhos vinculados</th><th>Acesso</th></tr></thead><tbody>${d.guardians.map(g=>`<tr><td><strong>${escapeHtml(g.name)}</strong></td><td>${escapeHtml(g.email)}<br><small>${escapeHtml(g.phone||'Telefone não informado')}</small></td><td><div class="inline-students">${(g.studentIds||[]).map(id=>{const student=students.find(s=>s.id===id);return student?`<span class="status-pill">${escapeHtml(student.name)} <button class="text-btn" type="button" title="Remover vínculo" data-unlink-user="${escapeHtml(g.id)}" data-unlink-student="${escapeHtml(id)}">×</button></span>`:''}).join('')||'Sem vínculo'}</div></td><td>${escapeHtml(g.status)}</td></tr>`).join('')}</tbody></table>`:'<p class="empty">Nenhum responsável cadastrado.</p>';
...
async function loadReports(){
  ...
  $('reportMetrics').innerHTML=[['▥',reports.length,'alunos no relatório'],['↘',totalEnt,'entradas registradas'],['↗',totalSai,'saídas registradas']].map(c=>`<article class="metric-card"><span class="metric-icon">${c[0]}</span><strong>${c[1]}</strong><span>${c[2]}</span></article>`).join('');
...
$('newStudentBtn').addEventListener('click',()=>{$('studentFormPanel').classList.remove('hidden');$('invitationResult').classList.add('hidden');$('editStudentPanel').classList.add('hidden');$('studentFormPanel').scrollIntoView({behavior:'smooth',block:'start'});$('newStudentName').focus()});
...
$('saveStudentBtn').addEventListener('click',async()=>{
  ...
  try{
    ...
    const payload={name:$('newStudentName').value,enrollment:$('newStudentEnrollment').value,classId:$('newStudentClass').value,
      ...(useExisting?{guardianId:$('studentGuardianId').value}:{guardian:{name:$('newGuardianName').value,email:$('newGuardianEmail').value,phone:$('newGuardianPhone').value,relationship:$('newGuardianRelation').value}})};
    const response=await api('/api/students',{method:'POST',body:JSON.stringify(payload)});
    $('studentFormPanel').classList.add('hidden');
    ['newStudentName','newStudentEnrollment','newGuardianName','newGuardianEmail','newGuardianPhone'].forEach(id=>$(id).value='');
...
$('saveLinkBtn').addEventListener('click',async()=>{const button=$('saveLinkBtn');button.disabled=true;try{
  if($('linkMode').value==='existing'){
    ...
    const result=await api('/api/guardians/invitations',{method:'POST',body:JSON.stringify({studentId:$('linkStudent').value,guardian:{name:$('linkGuardianName').value,email:$('linkGuardianEmail').value,phone:$('linkGuardianPhone').value,relationship:$('linkGuardianRelation').value}})});
...
$('guardianRegisterForm').addEventListener('submit',async event=>{event.preventDefault();const btn=$('registerGuardianBtn');btn.disabled=true;$('registerError').textContent='';
  try{const response=await api('/api/guardian/register',{method:'POST',body:JSON.stringify({email:$('regEmail').value,code:$('regCode').value,password:$('regPassword').value,confirmPassword:$('regConfirm').value})});
