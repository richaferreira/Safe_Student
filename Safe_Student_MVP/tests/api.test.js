...
test.beforeEach(() => { resetDb(); sessions.clear(); loginAttempts.clear(); passwordResets.clear(); });
...
test('recuperação de senha gera código temporário, redefine senha e invalida o código', async () => {
  const requested = await request('/api/password/forgot', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'gestor@demo.com' }),
  });
  assert.equal(requested.res.status, 200);
  assert.match(requested.body.demoCode, /^\d{6}$/);
  const changed = await request('/api/password/reset', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'gestor@demo.com', code: requested.body.demoCode, password: 'NovaSenha123', confirmPassword: 'NovaSenha123' }),
  });
  assert.equal(changed.res.status, 200);
  const logged = await request('/api/login', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'gestor@demo.com', password: 'NovaSenha123' }),
  });
  assert.equal(logged.res.status, 200);
  const reused = await request('/api/password/reset', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'gestor@demo.com', code: requested.body.demoCode, password: 'OutraSenha123', confirmPassword: 'OutraSenha123' }),
  });
...
test('CPF do responsável já ativo vincula o novo aluno automaticamente, sem convite', async () => {
  const manager = await login('gestor@demo.com');
  const { res, body } = await request('/api/students', {
    method: 'POST', headers: { 'Content-Type': 'application/json', ...auth(manager) },
    body: JSON.stringify({ name: 'Irmão Novo', enrollment: '2026-CPF1', classId: 'c1',
      guardian: { name: 'Responsável 1', email: 'outro-email-do-mesmo-responsavel@demo.com', phone: '22999990000', relationship: 'Mãe', cpf: '123.456.789-09' } }),
  });
  assert.equal(res.status, 201);
  assert.equal(body.guardianLinked, true);
  assert.equal(body.invitation, null);
  const guardian = await login('responsavel@demo.com');
  const mine = await request('/api/students', { headers: auth(guardian) });
...
test('CPF inválido é rejeitado no cadastro de aluno', async () => {
  ...
  const { res, body } = await request('/api/students', {
    method: 'POST', headers: { 'Content-Type': 'application/json', ...auth(manager) },
    body: JSON.stringify({ name: 'Estudante X', enrollment: '2026-CPFX', classId: 'c1',
      guardian: { name: 'Responsável Novo', email: 'novo.resp@demo.com', phone: '22988887777', relationship: 'Pai', cpf: '111.111.111-11' } }),
  });
...
test('CPF de responsável ativo com nome divergente é bloqueado por segurança', async () => {
  ...
  const { res, body } = await request('/api/students', {
    method: 'POST', headers: { 'Content-Type': 'application/json', ...auth(manager) },
    body: JSON.stringify({ name: 'Estudante Y', enrollment: '2026-CPFY', classId: 'c1',
      guardian: { name: 'Nome Diferente', email: 'outro2@demo.com', phone: '22988887777', relationship: 'Pai', cpf: '123.456.789-09' } }),
  });
  assert.equal(res.status, 409);
  assert.match(body.error, /nome/);
});
...
test('convite carrega CPF mascarado e ativação rejeita CPF inválido ou duplicado', async () => {
  ...
  const created = await request('/api/students', {
    method: 'POST', headers: { 'Content-Type': 'application/json', ...auth(manager) },
    body: JSON.stringify({ name: 'Estudante Z', enrollment: '2026-CPFZ', classId: 'c1',
      guardian: { name: 'Família Nova', email: 'familia.nova@demo.com', phone: '22977776666', relationship: 'Mãe', cpf: '987.654.321-00' } }),
  });
  assert.equal(created.res.status, 201);
  assert.equal(created.body.invitation.invitation.cpfMasked, '***.***.321-00');
  const code = created.body.invitation.activationCode;
  const badCpf = await request('/api/guardian/register', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'familia.nova@demo.com', code, password: 'senhaForte123', confirmPassword: 'senhaForte123', cpf: '111.111.111-11' }),
  });
  assert.equal(badCpf.res.status, 400);
  const dupeCpf = await request('/api/guardian/register', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'familia.nova@demo.com', code, password: 'senhaForte123', confirmPassword: 'senhaForte123', cpf: '123.456.789-09' }),
  });
  assert.equal(dupeCpf.res.status, 409);
  const ok = await request('/api/guardian/register', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'familia.nova@demo.com', code, password: 'senhaForte123', confirmPassword: 'senhaForte123' }),
  });
  assert.equal(ok.res.status, 201);
  assert.equal(ok.body.childrenLinked, 1);
});
