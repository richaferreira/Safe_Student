...
function seed() {
  return {
    school: {
      id: 'school_demo', name: 'Escola Teste', city: 'Saquarema - RJ', mode: 'DEMO'
    }, users: [ {
      ...
      id: 'demo', userId: 'resp1', profile: 'Responsável', scenario: 'Demo', success: true, timeSeconds: 10, score: 5, comment: 'Ilustrativo', source: 'DEMO_SEED', createdAt: '2026-06-18T14:20:00.000Z'
    }],
  };
}
function resetDb() {
...
test.beforeEach(() => {
  resetDb(); sessions.clear(); loginAttempts.clear();
});
...
test('diretório expõe somente dados mínimos do destinatário', async () => {
  ...
  = await request('/api/directory', {
    headers: auth(token)
  }); assert.ok(body.people.length > 0); for (const person of body.people) {
    assert.equal(Object.hasOwn(person, 'passwordHash'), false); assert.equal(Object.hasOwn(person, 'email'), false); assert.equal(Object.hasOwn(person, 'studentIds'), false);
  }
});
test('gestão não lê mensagem privada entre outros usuários', async () => {
  const resp = await login('responsavel@demo.com'); const sent = await request('/api/messages', {
    method: 'POST', headers: {
      'Content-Type': 'application/json', ...auth(resp)
    }, body: JSON.stringify({
      toUserId: 'port1', text: 'Mensagem privada entre responsável e portaria.'
    }),
  }); assert.equal(sent.res.status, 201);  const gest = await login('gestor@demo.com'); const messages = await request('/api/messages', {
    headers: auth(gest)
...
test('dashboard da gestão não recebe notificação destinada ao responsável', async () => {
  const port = await login('portaria@demo.com'); const attendance = await request('/api/attendance', {
    method: 'POST', headers: {
      'Content-Type': 'application/json', ...auth(port)
    }, body: JSON.stringify({
      token: 'SS-ALU001', type: 'ENTRADA'
    }),
  }); assert.equal(attendance.res.status, 201);  const gest = await login('gestor@demo.com'); const dashboard = await request('/api/dashboard', {
    headers: auth(gest)
...
test('CSV de validação exporta apenas evidências coletadas, sem DEMO_SEED', async () => {
  const resp = await login('responsavel@demo.com'); await request('/api/feedback', {
    method: 'POST', headers: {
      'Content-Type': 'application/json', ...auth(resp)
    }, body: JSON.stringify({
      profile: 'Responsável', scenario: 'Consulta', success: true, timeSeconds: 20, score: 4, comment: 'Coleta real de teste.'
    }),
  }); const gest = await login('gestor@demo.com'); const csv = await request('/api/feedback.csv', {
    headers: auth(gest)
  }); assert.equal(csv.res.status, 200); assert.match(csv.body, /Coleta real de teste/); assert.doesNotMatch(csv.body, /Ilustrativo/); assert.doesNotMatch(csv.body, /DEMO_SEED/);
});
test('restauração da demo gera evento de auditoria antes de invalidar a sessão', async () => {
  const gest = await login('gestor@demo.com'); const reset = await request('/api/demo/reset', {
    method: 'POST', headers: auth(gest)
  }); assert.equal(reset.res.status, 200); const db = readDatabase(); assert.equal(db.audit.some((a) => a.action === 'RESTAURAR_DEMO' && a.userId === 'gest1'), true);
});
